...
The TypeScript configuration reads `../atticd-config.toml` and supplies it to
Attic through `ATTIC_SERVER_CONFIG_BASE64` when Railway evaluates the file.
Apply the Railway configuration after editing the TOML to deploy the new settings.
Database, signing, and R2 credentials remain in Railway environment variables.

Attic uses Postgres for metadata and Cloudflare R2 for objects, so the Attic
container does not need a persistent volume. Keep the Postgres volume.
Applying this configuration detaches the old `atticd-volume`. After verifying
the deployment, delete that volume in the Railway dashboard; `config apply`
